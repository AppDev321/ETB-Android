...
import com.tharsol.endtb.ui.widget.RoundedButton
...
object Utilities
{
    ...
    var taskType = 0
    private val formatterDate: DateTimeFormatter = DateTimeFormat.forPattern("dd/MM/yyyy")
        .withLocale(Locale.ENGLISH)
    private val formatterDate1: DateTimeFormatter = DateTimeFormat.forPattern("yyyy-MM-dd")
        .withLocale(Locale.ENGLISH)
    private val formatterTime1: DateTimeFormatter = DateTimeFormat.forPattern("HH:mm")
        .withLocale(Locale.ENGLISH)
    private val formatterTime: DateTimeFormatter = DateTimeFormat.forPattern("hh:mm aa")
        .withLocale(Locale.ENGLISH)
    private val formatterTimeWithSeconds: DateTimeFormatter = DateTimeFormat.forPattern("HH:mm:ss")
        .withLocale(Locale.ENGLISH)
    private val formatterCompleteDayMonth: DateTimeFormatter = DateTimeFormat.forPattern("EEEE, dd MMMM, yyyy")
        .withLocale(Locale.ENGLISH)
    ...
    fun convertDateFormat(date: String?): String
    {
        return try
        {
            formatterDate.print(formatterDate1.parseDateTime(date))
        }
        catch (e: Exception)
        {
            formatterDate1.print(formatterDate.parseDateTime(date))
        }
    }

    fun convertDateToDayMonthNameFormat(date: String?): String
    {
        return try
        {
            formatterCompleteDayMonth.print(formatterDate1.parseDateTime(date))
        }
        catch (e: Exception)
        {
            formatterCompleteDayMonth.print(formatterDate.parseDateTime(date))
        }
    }
    ...
    fun convertTimeFormat(time: String?): String
    {
        return try
        {
            formatterTime.print(formatterTime1.parseDateTime(time))
        }
        catch (e: Exception)
        {
            e.printStackTrace()
            formatterTime.print(formatterTimeWithSeconds.parseDateTime(time))
        }
    }

    fun isAllowScreenCapture(context: Context): Boolean
    {
        try
        {
            if (ALLOW_SCREEN_CAPTURE_CHECKED)
            {
                return ALLOW_SCREEN_CAPTURE
            }
            val sharedPreferences = getSharedPreferences(context)
            ALLOW_SCREEN_CAPTURE = sharedPreferences.getBoolean(Constants.ALLOW_SCREEN_CAPTURE,
                    false)
            ALLOW_SCREEN_CAPTURE_CHECKED = true
            return ALLOW_SCREEN_CAPTURE
        }
        catch (ex: Exception)
        {
    ...
    @SuppressLint("HardwareIds") fun getDeviceUdId(context: Context): String?
    {
        if (TextUtils.isEmpty(mDeviceUdId))
        {
            mDeviceUdId = Settings.Secure.getString(context.contentResolver, Settings.Secure.ANDROID_ID)
        }
        return mDeviceUdId
    }

    /*private fun getDeviceUdIdPre10(context: Context): String
    {
        ...
        return if (tm != null) tm.deviceId else ""
    }*/

    private fun getDeviceUdId10(context: Context): String
    {
        val storage = Storage(App.context)
        storage.setEncryptConfiguration(StorageUtils.config)
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
        {
            return ""
        }
        val path: String = StorageUtils.devicePath
        if (!storage.isFileExist(path)) return createDeviceId()
        val id: String = storage.readTextFile(path)
        return if (TextUtils.isEmpty(id)) createDeviceId() else id
    }
    ...
    private fun createDeviceId(): String
    {
        val storage = Storage(App.context)
        storage.setEncryptConfiguration(StorageUtils.config)
        val uid: String = UDIDUtils.generateRandom()
        storage.createFile(StorageUtils.devicePath, uid)
        return uid
    }
    ...
    fun setPostSpan(textView: TextView, linkText: String?, clickableSpanListener: ClickableSpan?)
    {
        ...
        spannable.setSpan(clickableSpanListener,
                startIndex,
                endIndex,
    ...
    @JvmOverloads fun changeButtonBackground(button: Button,
                                             color: Int,
                                             supportOldStyle: Boolean = false)
    {
    ...
    @JvmOverloads fun changeButtonBackground(button: RoundedButton,
                                             color: Int,
                                             supportOldStyle: Boolean = false)
    {
        if (Build.VERSION.SDK_INT > 19 || !supportOldStyle)
    ...
    fun isInternetConnected(context: Context, view: View?, notify: Boolean): Boolean
    {
        ...
        if (notify && isDisconnected)
        {
            showSnackbar(view,
                    ...
                    context.getString(R.string.retry)) { v: View? ->
                isInternetConnected(context, view, true)
            }
        }
    ...
    //    }
    @JvmOverloads fun withSuffix(count: Long, oneDigit: Boolean = true): String
    {
        if (count < 1000) return "" + count
        val charc = "KMBT"
        val exp = (Math.log(count.toDouble()) / Math.log(1000.0)).toInt()
        val value = count / Math.pow(1000.0, exp.toDouble())
        return if (oneDigit) String.format(Locale.ENGLISH, "%.1f%c", value, charc[exp - 1])
    ...
    @Throws(IOException::class) fun getConnectivityStatusString(context: Context): String?
    {
    ...
    fun turnGPSOn(context: Context)
    {
        try
        {
            ...
            val provider = Settings.Secure.getString(context.contentResolver,
    ...
    fun getForegroundScreen(context: Context): String?
    {
        ...
        {
            @SuppressLint("WrongConstant") val mUsageStatsManager = context.getSystemService("usagestats") as UsageStatsManager
            val time = System.currentTimeMillis() // We get usage stats for the last 10 seconds
            val stats = mUsageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY,
                    time - 1000 * 10,
                    time) // Sort the stats by the last time used
            if (stats != null)
            {
    ...
    fun getBatteryLevel(context: Context): Float
    {
        try
        {
            ...
            val scale = batteryIntent.getIntExtra(BatteryManager.EXTRA_SCALE,
                    -1) // Error checking that probably isn't needed but I added just in case.
    ...
    fun setTextAnimation(context: Activity, textView: TextView)
    {
        ...
        val animation: Animation = TranslateAnimation(width.toFloat(),
    ...
    {
        val width = getScreenSize(context).widthPixels
        val animation = TranslateAnimation(width.toFloat(),
    ...
    {
        val width: Int = Utilities.getScreenSize(context).widthPixels
        val animation: Animation = TranslateAnimation((width / 2).toFloat(),
                -1f * Utilities.getTextViewContentsWidth(textView),
                0f,
                0f)
    ...
    { //        ServiceUrls.setCloudServerUrl(null)
        ...
        //        AppDb.setNull()
        Utilities.getSharedPreferences(App.context).edit().clear()
            .apply() //        Utilities.setUser(null)
    ...
    fun getTimeDifference(serverDateTime: String?): Int
    {
        try
        {
            val takenTime: DateTime = DateTime.parse(serverDateTime,
            ...
            val seconds = difference.standardSeconds.toInt()
            printDebug(" difference $seconds")
            return seconds
        }
        catch (e: Exception)
        {
    ...
    fun getLastSyncDaysDuration(context: Context): Int
    {
        try
        {
            val preferences = getSharedPreferences(context)
            val lastSyncOn = preferences.getString(Constants.LAST_SYNC_DATE, "")
            if (!TextUtils.isEmpty(lastSyncOn))
            {
                val lastSyncDate: DateTime = DateTime.parse(lastSyncOn,
                        DateTimeFormat.forPattern("yyyy-MM-dd"))
                return Days.daysBetween(lastSyncDate.toLocalDate(),
                        DateTime.now().toLocalDate()).days
            }
        }
        catch (ex: Exception)
        {
    ...
    private fun lcs(text1: String, text2: String): String
    {
        ...
        for (i in 0 until text1.length) for (j in 0 until text2.length) if (text1[i] == text2[j]) lengths[i + 1][j + 1] = lengths[i][j] + 1
    ...
    {
        val theta = lon1 - lon2
        var dist = Math.sin(deg2rad(lat1)) * Math.sin(deg2rad(lat2)) + Math.cos(deg2rad(lat1)) * Math.cos(
    ...
    {
        return !TextUtils.equals(Settings.Secure.getString(context.contentResolver,
    ...
    {
        return HtmlCompat.fromHtml(context.getString(R.string.month) + ": <b><u>" + monthYear + "</u></b>",
                HtmlCompat.FROM_HTML_MODE_LEGACY)
    }

    val uriAuthority: String
        get() = BuildConfig.APPLICATION_ID.toString() + ".fileprovider"
